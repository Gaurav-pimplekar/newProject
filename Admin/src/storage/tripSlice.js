...
export const fetchTripByPairId = createAsyncThunk(
    ...
    async (pairId) => {
        try {
            const response = await axios.get(`http://192.168.1.11:8080/trips/pair/${pairId}`);
...
export const sendOtp = createAsyncThunk('otp/sendOtp', async () => {
    const response = await axios.post('http://192.168.1.11:8080/send-otp', { email: "gpimplekar@gmail.com" });
...
export const verifyOtp = createAsyncThunk('otp/verifyOtp', async ({ email, otp }, { rejectWithValue }) => {
    try {
        ...
        const response = await axios.post('http://192.168.1.11:8080/verify-otp', { email, otp });
...
export const fetchUserDetails = createAsyncThunk(
    ...
    async ({ mobileNumber }) => {
        try {
            const response = await axios.get(`http://192.168.1.11:8080/driverByNumber/${mobileNumber}`);
...
export const loginDriver = createAsyncThunk(
    "logindriver", async () => {
        try {
            const res = await axios.get("http://192.168.1.11:8080/login/driver", {
                headers: {
